...
public class Conrec
{
    ...
    /// Contour level
    /// </param>
    public delegate void RendererDelegate(float x1, float y1, float x2, float y2, float z);
    ...
    /// The renderer.
    /// </param>
    public static void Contour(float[,] d, float[] x, float[] y, float[] z, RendererDelegate renderer)
    {
        float x1 = 0.0f;
        float x2 = 0.0f;
        float y1 = 0.0f;
        ...
        Func<int, int, float> xsect = (p1, p2) => (h[p2] * xh[p1] - h[p1] * xh[p2]) / (h[p2] - h[p1]);
        Func<int, int, float> ysect = (p1, p2) => (h[p2] * yh[p1] - h[p1] * yh[p2]) / (h[p2] - h[p1]);
        ...
        for (int j = jub - 1; j >= jlb; j--)
        {
            ...
            for (i = ilb; i <= iub - 1; i++)
            {
                float temp1 = Math.Min(d[i, j], d[i, j + 1]);
                float temp2 = Math.Min(d[i + 1, j], d[i + 1, j + 1]);
                float dmin = Math.Min(temp1, temp2);
                temp1 = Math.Max(d[i, j], d[i, j + 1]);
                temp2 = Math.Max(d[i + 1, j], d[i + 1, j + 1]);
                float dmax = Math.Max(temp1, temp2);
